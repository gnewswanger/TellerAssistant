...
#Region "Class Properties"
    ...
    Public Function NextCheck(value As CheckStatus) As ChecksClass
        If (Me._checkQueue.Count - 1) > Me._queueIndex Then
            Me._queueIndex += 1
        End If
        Return Me.CurrentQueueCheck
    End Function
    ...
    Public Function PreviousCheck(value As CheckStatus) As ChecksClass
        If (Me._checkQueue.Count - 1) >= Me._queueIndex Then
            Me._queueIndex -= 1
        End If
        Return Me.CurrentQueueCheck
...
#End Region
    ...
    Friend Function AddCheck(ByVal chkEvent As CheckDataEventArgs) As CheckRegisterEventArgs
        If chkEvent.Check.Status = Me._queueCheckStatus Then
            ...
            If Me._checkQueue.Count = 1 Then
                Me._queueIndex = 0
            End If
            Return New CheckRegisterEventArgs(EventName.envmCurrentQueueCheckChanged, Me._checkQueue(Me.CheckQueueIndex), _
                                              Me._checkQueue(Me.CheckQueueIndex), Me._checkQueue.Count, Me.CheckQueueIndex)
        End If
        Return Nothing
    ...
    Friend Function RemoveCheck(ByVal chkEvent As CheckDataEventArgs) As CheckRegisterEventArgs
        For i As Integer = 0 To Me._checkQueue.Count - 1
            ...
            AndAlso Me._checkQueue.Item(i).CheckNo.Trim = chkEvent.Check.CheckNo.Trim Then
                ...
                Return New CheckRegisterEventArgs(EventName.envmCurrentQueueCheckChanged, Me.CurrentQueueCheck, Me.CurrentQueueCheck, Me.CheckQueueCount, Me.CheckQueueIndex)
                'NotifyObservers(Me, New CheckRegisterEventArgs(EventName.envmCurrentQueueCheckChanged, Me.CurrentQueueCheck, Me.CurrentQueueCheck, Me.CheckQueueCount, Me.CheckQueueIndex))
                Exit For
            End If
        Next
    ...
    Friend Function UpdateDonorInfo(ByVal donorInfo As DonorInfoEventArgs) As Boolean
        If donorInfo.CheckQueue = Me._queueCheckStatus Then
